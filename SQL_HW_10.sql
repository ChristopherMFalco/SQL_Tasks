...
SELECT last_name, COUNT(last_name) AS 'Number of Actors With The Last Name' 
FROM actor
GROUP BY last_name;
...
-- 4c. HARPO WILLIAMS was accidentally entered in the actor table as GROUCHO WILLIAMS, 
...
SELECT category.name, sum(payment.amount) as 'Revenue per Category' FROM category
...
CREATE VIEW the_top_5_by_genres AS
SELECT category.name, sum(payment.amount) as 'Revenue per Category' FROM category
INNER JOIN film_category
ON category.category_id = film_category.category_id
INNER JOIN inventory
...
SELECT * FROM the_top_5_by_genres;

-- 8c. You find that you no longer need the view top_five_genres. Write a query to delete it.
DROP VIEW the_top_5_by_genres;